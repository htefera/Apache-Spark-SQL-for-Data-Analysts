...
select count(*) from eventsRaw where ecommerce.purchase_revenue_in_usd IS NOT NULL;



-- COMMAND ----------

select count(*) from eventsRaw;
...
(
SELECT 
ecommerce.total_item_quantity as purchases,
cast(from_unixtime(event_previous_timestamp / 1000000) as timestamp) AS previousEventDate,
cast(from_unixtime(event_timestamp / 1000000) as timestamp) AS EventDate, ---- alternative  to_date(event_timestamp) as EventDate
...
SELECT*FROM tempData ORDER BY purchases desc;
...
--TODO
select city , state, ecommerce.purchase_revenue_in_usd as purchase from purchaseEvents order by purchase desc;
