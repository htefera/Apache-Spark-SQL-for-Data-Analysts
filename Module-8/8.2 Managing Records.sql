...
-- MAGIC `line 6`: `LAG` gets the `heartrate` from the previous row. We define the window by `device_id` and `date` so that the calculation is applied to each missing value **by device** <br>
...
-- update the silver table with the interpolated values
...
Select* from health_tracker_data_2020_02_late;
...
-- VERSION 0 
-- Version 0 shows the data after we added records from January.
...
-- VERSION 1
-- Version 1 shows the data after we added records from February.
SELECT COUNT(*) FROM health_tracker_silver VERSION AS OF 1

-- COMMAND ----------

-- CURRENT VERSION
-- Current version shows the data after we added the interpolated records by merging with the previous silvertable  .
SELECT COUNT(*) FROM health_tracker_silver
