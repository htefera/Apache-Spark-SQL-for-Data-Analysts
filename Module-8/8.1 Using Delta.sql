...
-- this is called a bronze level table 
...
-- TODO only data for the Junaury month. This is called bronze level table 
SELECT * FROM health_tracker_data_2020_01 TABLESAMPLE (40 percent)
...
-- TODO creating a cleaned Silver delat table

CREATE OR REPLACE TABLE health_tracker_silver 
USING DELTA
PARTITIONED BY (p_device_id)
LOCATION "/health_tracker/silver" --location where we need to place our delta table
AS (
SELECT
  value.name,
  value.heartrate,
  CAST(FROM_UNIXTIME(value.time) AS timestamp) AS time,
  CAST(FROM_UNIXTIME(value.time) AS DATE) AS date,
...
select count(*) from health_tracker_silver where p_device_id=4;
...
--TODO table that holds data for the second month-Febraury
...
--Insert the data of Febraury into the Silver table 
INSERT INTO
...
SELECT
  ...
  value.device_id p_device_id
FROM
  health_tracker_data_2020_02   -- Table created for the month of Febraury
...
-- Display the count before append
...
-- The dates of 27, 28 and 29 of Febrauary are those where the device 4 were not recording the heartrate properly
...
AS 
(
  SELECT COUNT(*) as broken_readings_count, date FROM health_tracker_silver
  WHERE heartrate < 0
  GROUP BY date
  ORDER BY date
)
