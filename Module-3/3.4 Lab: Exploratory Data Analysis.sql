...
select *from sales where countryName="Italy";
...
unitPrice,
countryName
FROM outdoorproducts where quantity >0; 
...
-- TODO 
-- unique records 
-- select DISTINCT countryName, quantity, unitPrice, stockCode, totalAmount from sales; 
-- unique countryname values 
...
-- TODO Create a temporary view: salesQuants

CREATE OR REPLACE TEMPORARY VIEW salesQuants as
SELECT SUM(quantity) as totalQuantity, countryName from sales
group by countryName order by totalQuantity;
...
select * from salesQuants order by totalquantity desc;
...
-- TODO  Read in a new parquet table
...
--TODO Join and Visualize

SELECT totalQuantity, alpha3Code as CountryCode from salesQuants JOIN countryCodes on EnglishShortName= countryName limit 100;
...
-- TODO Display a distinct list of all countries in the countryCodes table
SELECT DISTINCT ENGLISHSHORTNAME AS Name FROM COUNTRYCODES;


-- COMMAND ----------

-- TODO Display a distinct list of all countries in the outdoorProducts table
...
-- TODO select countrynames that exists in both tables;

SELECT distinct countryName FROM outdoorproducts JOIN COUNTRYCODES ON countryName= ENGLISHSHORTNAME;
